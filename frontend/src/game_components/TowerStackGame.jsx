...
const TowerStackGame = () => {
  ...
  class Stage {
    constructor() {
      const canvasWidth =
        document.getElementsByClassName("tower-game")[0].offsetWidth;
      const canvasHeight =
        document.getElementsByClassName("tower-game")[0].offsetHeight;
      // container
      ...
      refContainer.current &&
        refContainer.current.appendChild(this.renderer.domElement);
      // scene
    ...
    onResize() {
      ...
      let viewSize = 30;
      this.renderer.setSize(canvasWidth, canvasHeight);
      this.camera.left = canvasWidth / -viewSize;
      this.camera.right = canvasWidth / viewSize;
      this.camera.top = canvasHeight / viewSize;
      this.camera.bottom = canvasHeight / -viewSize;
  ...
  class Game {
    ...
    addBlock() {
      ...
      if (lastBlock && lastBlock.state == lastBlock.STATES.MISSED) {
        score = parseInt(this.scoreContainer.innerHTML);
        saveData();
        return this.endGame();
      }
  ...
  const saveData = async () => {
    try {
      const res = await updateRecord({ gameId: "towerGame", score });
    } catch (err) {
  ...
  return (
    <div className="tower-game">
      <div id="container">
        <div ref={refContainer} id="game"></div>
        <div id="score">0</div>
        <div id="instructions">
