...
const FlappyBird = () => {
  ...
  const saveData = async (score) => {
    try {
      const res = await updateRecord({ gameId: "jumpGame", score });
    } catch (err) {
  ...
  useEffect(() => {
    ...
    var fgSVG = `
            <svg id="fg-svg" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns="http://www.w3.org/2000/svg"
                height="110" viewBox="0 0 360 110" width="360" version="1.1" xmlns:cc="http://creativecommons.org/ns#"
    ...
    function checkCollision() {
      ...
      if (
        ...
        (dinoPos_Y <= bones[bonePtr].y + topBone.height ||
          dinoPos_Y + hitboxHeight >=
            bones[bonePtr].y + topBone.height + flyGap)
      ) {
        setGameOver();
      }
    }
    ...
    function render() {
      ...
      for (var i = 0; i < bones.length; i++) {
        ...
        ctx.drawImage(
          botBone,
          bones[i].x,
          bones[i].y + topBone.height + flyGap
        );
      }
  ...
  return (
    <>
      <div className="flappy-bird">
        <div id="ctn" className="ctn">
          <canvas id="c"></canvas>
          <div id="scoreboard">0</div>
          <div id="start-screen">
            <div id="ctrl-ctn">
              <div id="up" className="ctrl-btn">
                ↑
              </div>
              <div id="space" className="ctrl-btn">
                SPACE
              </div>
              <div id="tap" className="ctrl-btn">
                TAP
              </div>
            </div>
          </div>
